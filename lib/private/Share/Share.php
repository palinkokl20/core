	...
	public static function getUsersSharingFile($path, $ownerUser, $includeOwner = false, $returnUserPaths = false, $recursive = true) {
		...
		while ($source !== -1) {
			...
			if (\OCP\DB::isError($result)) {
				...
				while ($row = $result->fetchRow()) {
					$usersInGroup = \OC::$server->getGroupManager()->usersInGroup($row['share_with']);
		...
		if ($items) {
			foreach ($items as $item) {
				if ((int)$item['share_type'] === self::SHARE_TYPE_USER) {
					...
					$users = array_merge($users, \OC::$server->getGroupManager()->usersInGroup($item['share_with']));
				}
			}
		}
		return $users;
	...
	private static function put($itemType, $itemSource, $shareType, $shareWith, $uidOwner,
		...
		if ($shareType == self::SHARE_TYPE_GROUP) {
			...
			if (isset($shareWith['users'])) {
				...
				$users = \OC::$server->getGroupManager()->usersInGroup($shareWith['group']);
			}
